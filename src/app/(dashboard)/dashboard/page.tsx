...
const Dashboard = () => {
  ...
  return (
    <div>
    {
      isLoading ? <p className='my-4 w-fit mx-auto'>Loading.......</p> :<>
      {
        ...
        ?<div className='grid gap-6 lg:grid-cols-2 p-4 lg:p-6'>
          {
            data?.map((item,index)=>{
              return (
                <Card onClick={()=>handleRedirectToEditorPage(item?._id)} key={item?._id} className='cursor-pointer overflow-hidden group max-h-60'>
                  <CardHeader>
                    <CardTitle>
                      {item?.name}
                    </CardTitle>
                    ...
                    <CardContent >
                      <div className="border min-h-60 overflow-hidden rounded-lg top-15 group-hover:top-4 transition-all relative shadow drop-shadow-2xl ">
                        <iframe className='w-full h-full rounded-md' src={`${process.env.NEXT_PUBLIC_BASE_URL}/api/file/${item?._id}/index.html`}></iframe>
        ...
        : <div className='flex min-h-[calc(100vh-3rem)]  flex-col items-center justify-center'>
          <Image src={"/project.svg"} width={300} height={300} alt='Create Project'/>
